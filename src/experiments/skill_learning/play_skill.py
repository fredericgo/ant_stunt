...
def render_trajectory(policy, env, writer, s):

    state = env.reset()
    skill = torch.tensor(s)#sample_skill()
    skill = skill.unsqueeze(0)

    title_font = ImageFont.truetype('dejavu/DejaVuSerif-Bold.ttf', 200)
    title_text = str(s)
...
def main():
    ...
    for i in range(args.num_skills):
        render_trajectory(policy, env, writer, i)
    env.close()
