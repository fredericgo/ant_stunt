...
from sac_kl.sac_kl import SACKL
...
parser.add_argument('--kl_weight', type=float, default=1e-7, metavar='G',
                    help='Temperature parameter α determines the relative importance of the entropy\
                            term against the reward (default: 0.2)')
...
parser.add_argument('--pretrained_policy', default="data/policy/mepol_pretrained_policy.torch",
                    help='Policy Type: Gaussian | Deterministic (default: Gaussian)')
parser.add_argument('--metric_type', default="angle",
                    help='metric type')
parser.add_argument('--target_q', type=list, default=[1.0, 0.0, 0.0, 0.0],
                    help='target rotation (quaternion)')
