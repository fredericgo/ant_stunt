...
class SACMepol(object):
    def __init__(self, env, args):
        ...
        self.speed_level = args.speed_level
        ...
        self.log_dir = f'runs/sacmepol/{datetime_st}_SACMepol_{args.env_name}'
    ...
    def compute_entropy(self, states, state_filter, k, num_workers):
        states = states[:, state_filter]
        distances, indices = self.compute_knn(states, k, num_workers)
        # Compute objective function
        # compute volume for each particle
        volumes = (np.power(distances[:, self.k], self.ns) * np.power(np.pi, self.ns/2)) / self.G
        # compute entropy
        entropy = - np.mean(np.log((self.k / (self.traj_len * volumes + EPS)) + EPS)) + self.B
        return entropy

    def compute_penalty(self, states, velocity_filter=[18, 19, 20, 21, 22, 23]):
        vel = states[:, velocity_filter]
        v = np.sqrt(np.sum(vel ** 2, axis=-1))
        v = np.maximum(v, self.speed_level) - self.speed_level
        return v
    ...
    def train(self):
        ...
        while epoch < self.num_epochs:
            ...
            entropy = self.compute_entropy(next_states, self.state_filter, self.k, self.num_workers)
            penalty = self.compute_penalty(next_states)

            rewards = entropy - self.penalty_weight * penalty
            ...
            if epoch % self.log_interval == 0:
                self.writer.add_scalar('reward/train', rewards.mean(), epoch)
