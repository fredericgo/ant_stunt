...
class QNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_sizes):
        ...
        num_features = num_inputs + num_actions
        # Q1 architecture
        layers = []
        layers.extend((nn.Linear(num_features, hidden_sizes[0]), nn.ReLU()))
...
class ConditionalGaussianPolicy(nn.Module):
    def __init__(
        ...
        action_space,
        log_std_init=-0.5):
        super(ConditionalGaussianPolicy, self).__init__()

        self.activation = nn.ReLU
        self.num_skills = num_skills

        num_features = num_inputs + num_skills
        layers = []
    ...
    def forward(self, state, skill):
        k = F.one_hot(skill, self.num_skills)
        input_ = torch.cat([state, k], 1)
    ...
    def to(self, device):
        self.action_scale = self.action_scale.to(device)
        self.action_bias = self.action_bias.to(device)
        return super(ConditionalGaussianPolicy, self).to(device)
...
class DiscriminatorNetwork(nn.Module):
    def __init__(self, num_inputs, n_classes, hidden_sizes):
